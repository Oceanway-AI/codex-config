...
如果您之前使用旧版本软件完成过配置，当前状态可能显示“图片备用：待补全”。此时不需要重新复制已保存的 API Key，直接点击“补全图片备用配置”，再完全退出并重新打开 Codex 即可。重复点击只会同步最新的 Key 和 Base URL，不会重复添加配置。
...
这通常表示您曾使用旧版本工具配置 OceanWay，但还没有写入 imagegen CLI 备用环境。点击“补全图片备用配置”，然后完全退出并重新打开 Codex 即可。
