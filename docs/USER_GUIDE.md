...
如果您有多个 API Key，例如“订阅密钥”和“余额密钥”，可以在“密钥名称”中填写名称，再点击“保存密钥”。

之后只需要在“密钥档案”中选择对应密钥，点击“启用”，软件会根据您当前是否已登录 ChatGPT 自动选择配置方式。

已保存的密钥会保存在本机 Codex 目录的 `oceanway-ai-keys.json` 文件中，软件界面只显示脱敏后的密钥。删除最后一个密钥档案后，这个文件也会被删除。

如果删除的是当前正在使用的密钥档案，软件会同时清除 Codex 当前 OceanWay 配置和本工具写入的密钥配置。
