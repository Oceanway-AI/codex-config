...
“一键配置并重启”会写入直接图片 API 的任务规则。默认图片模型为 `gpt-image-2`，用户明确指定其他模型时按指定调用。

- 普通生图通过当前服务的 `/images/generations`。
- 参考图通过 `/images/edits` 上传原图。参考图直接放在 Codex 对话中，无需再传给配置软件。
- 用户要求几张就安排几张；未指定才默认一张。并发最多两个请求不代表总数限制。

不依赖 imagegen、内置图片工具、专用图片 CLI 或新增 MCP。规则写入配置与全局任务指令文件，并保留已有内容。无需在每次请求中重复指定调用路径。配置保存后请重启并新建任务；任务的更高优先级要求仍然生效。
...
“恢复原配置”恢复首次快照中的认证和 provider，移除全局任务指令中本工具管理的图片规则，保留其他指令。存在本工具记录的历史迁移时仍按原规则撤销，不新增跨供应商会话兼容保证。
...
这表示直接图片规则尚未完整写入。保持 Key 输入框为空，重新执行一键配置，然后新建 Codex 任务。规则已保存不等于图片接口或自然语言流程已经通过实测。
